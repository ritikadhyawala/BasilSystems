...
public class MqttService extends Service implements MqttSimpleCallback {
	...
	public void onCreate() {
		...
		// added by ritika
		 brokerHostName = "192.168.1.247";
//		brokerHostName = "ec2-52-11-122-248.us-west-2.compute.amazonaws.com";
		topicName = "emailId";
		// end
	...
	synchronized void handleStart(Intent intent, int startId) {
		...
		if (isAlreadyConnected() == false) {
			...
			// Notification(R.drawable.abc_btn_check_material,
			// "MQTT",
			...
			if (isOnline()) {
				...
				if (connectToBroker()) {
					...
					subscribeToTopic("getUser");
					subscribeToTopic("getModes");
					subscribeToTopic("getUserPlacesNames");
				}
			} else {
	...
	private void broadcastReceivedMessage(String topic, String message) {
		...
		// in the app GUI
		if ("getUser".equals(topic)) {
			Intent broadcastIntent = new Intent();
			broadcastIntent.setAction("getUserData");
			broadcastIntent.putExtra("getUser", message);
			sendBroadcast(broadcastIntent);
		} else if ("getModes".equals(topic)) {
			Intent broadcastIntent = new Intent();
			broadcastIntent.setAction("getUserData");
			broadcastIntent.putExtra("getModes", message);
			sendBroadcast(broadcastIntent);
		} else if ("getUserPlacesNames".equals(topic)) {
			Intent broadcastIntent = new Intent();
			broadcastIntent.setAction("getUserData");
			broadcastIntent.putExtra("getUserPlacesNames", message);
			sendBroadcast(broadcastIntent);
		} else {
			// Intent broadcastIntent = new Intent();
			...
			// sendBroadcast(broadcastIntent);
			broadcastToActivity(getApplicationContext(), message);
		}
	}

	private void broadcastToActivity(Context context, String message) {
		...
		try {
			...
			if (applianceStatus.has("o")) {
				...
				messageForUpdate.put("isOn",
						applianceStatus.getInt("o") > 0 ? true : false);
			} else if (applianceStatus.has("a")) {
				messageForUpdate.put("isAuto",
						applianceStatus.getInt("a") > 0 ? true : false);
			} else {
				messageForUpdate.put("status", applianceStatus.getInt("l") * 6);

			}
			DatabaseHandler.getInstance(context).updateAppliance(
					applianceStatus.getString("s"), messageForUpdate);
	...
	private void notifyUser(String alert, String title, String body) {
		...
		// getSystemService(NOTIFICATION_SERVICE);
		// Notification notification = new
		// Notification(R.drawable.abc_btn_check_material, alert,
		// System.currentTimeMillis());
	...
	private String generateClientId() {
		...
		// using hardcode value of client id
		mqttClientId = "emailId";
		return mqttClientId;
	}
	...
	public void publishMessage(String topic, byte[] message) {
		if (mqttClient.isConnected()) {
			try {
				...
				Log.d("MQTT Send error", e.getMessage());
				e.printStackTrace();
			} catch (MqttPersistenceException e) {
				Log.d("MQTT Send error", e.getMessage());
				e.printStackTrace();
